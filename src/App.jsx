...
function SyncDot({ synced }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)' }}>{synced ? 'Live' : 'Offline'}</span>
    </div>
  );
}
...
function NameScreen({ onJoin }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0F0F1A', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '1.5rem' }}>
      <div style={{ maxWidth: 380, width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <div style={{ fontSize: 52, marginBottom: 12 }}>🏆</div>
          <h1 style={{ color: '#fff', fontSize: 26, fontWeight: 700, letterSpacing: '-0.5px' }}>Netcore Battle Arena</h1>
          <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: 14, marginTop: 6 }}>Indoor Games Tournament · July 2025</p>
        </div>

        <div style={{ background: '#18182A', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: '1.75rem 1.5rem' }}>
          <div style={{ display: 'flex', gap: 6, marginBottom: 20, background: 'rgba(255,255,255,0.05)', borderRadius: 10, padding: 4 }}>
            {['user', 'admin'].map(m => (
              <button key={m} onClick={() => setMode(m)} style={{ flex: 1, padding: '7px 0', borderRadius: 8, border: 'none', background: mode === m ? '#7C6EE8' : 'transparent', color: mode === m ? '#fff' : 'rgba(255,255,255,0.4)', fontSize: 13, fontWeight: 600, cursor: 'pointer', textTransform: 'capitalize' }}>{m === 'admin' ? '⚙️ Admin' : '🎯 Predictor'}</button>
            ))}
          </div>
...
function Field({ label, children, style: s }) {
  return (
    <div style={s}>
      <label style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', display: 'block', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.6px' }}>{label}</label>
      {children}
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0F0F1A', paddingBottom: 80 }}>

      {/* ── Header ── */}
      <div style={{ background: '#18182A', borderBottom: '0.5px solid rgba(255,255,255,0.08)', padding: '0.9rem 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50 }}>
        <div>
          <div style={{ fontSize: 16, fontWeight: 700, letterSpacing: '-0.3px' }}>🏆 Netcore Battle Arena</div>
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', marginTop: 1 }}>July 2025 · Indoor Games</div>
        </div>
      ...
      {/* ── Stats strip ── */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', borderBottom: '0.5px solid rgba(255,255,255,0.06)' }}>
        {[['Predicted', predCount], ['Correct', myCorrect], ['Rank', myRank ? `#${myRank}` : '—'], ['Points', myPoints]].map(([label, val]) => (
          <div key={label} style={{ padding: '10px 0', textAlign: 'center', borderRight: '0.5px solid rgba(255,255,255,0.06)' }}>
            <div style={{ fontSize: 19, fontWeight: 700, color: '#A89DF0' }}>{val}</div>
            <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.3)', marginTop: 2, textTransform: 'uppercase', letterSpacing: '0.6px' }}>{label}</div>
          </div>
        ))}
      </div>
        ...
        {view === 'predict' && (
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
              {['All', ...GAMES].map(g => (
                <button key={g} onClick={() => setFilterGame(g)} style={{ padding: '5px 14px', borderRadius: 999, border: '0.5px solid rgba(255,255,255,0.15)', background: filterGame === g ? '#7C6EE8' : 'transparent', color: filterGame === g ? '#fff' : 'rgba(255,255,255,0.45)', fontSize: 13, cursor: 'pointer' }}>{g}</button>
              ))}
            </div>
            ...
            {submitted && (
              <div style={{ background: 'rgba(124,110,232,0.1)', border: '0.5px solid rgba(124,110,232,0.3)', borderRadius: 10, padding: '10px 14px', marginBottom: 14, fontSize: 13, color: '#A89DF0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button onClick={() => { setSubmitted(false); localStorage.removeItem('nba_submitted'); }} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.35)', fontSize: 12, cursor: 'pointer' }}>Edit</button>
              </div>
            )}
        ...
        {view === 'results' && (
          <div>
            ...
            {completed.map(m => {
              ...
              return (
                <div key={m.id} style={{ background: '#18182A', border: '0.5px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '1rem 1.25rem', marginBottom: 10 }}>
                  <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                    <Badge label={m.game} style={GAME_COLORS[m.game]} />
                    <Badge label={m.type} style={TYPE_COLORS[m.type]} />
                    <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', marginLeft: 'auto' }}>{m.round}</span>
                  </div>
                  <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', marginBottom: 6 }}>{m.p1.name} vs {m.p2.name}</div>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    ) : <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.2)' }}>No prediction</span>}
                  </div>
                </div>
              );
            })}
        ...
        {view === 'leaderboard' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              ...
              <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)' }}>Refreshes every 8s</span>
            </div>
            ...
            {sortedPlayers.length >= 3 && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 20, alignItems: 'flex-end' }}>
                {[sortedPlayers[1], sortedPlayers[0], sortedPlayers[2]].map((p, i) => {
                  ...
                  const medals  = ['🥈', '🥇', '🥉'];
                  return (
                    <div key={p.id} style={{ background: '#18182A', border: `0.5px solid ${p.id === me.id ? 'rgba(124,110,232,0.4)' : 'rgba(255,255,255,0.07)'}`, borderRadius: 12, padding: '12px 8px', textAlign: 'center', height: heights[i], display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                      <div style={{ fontSize: 20, marginBottom: 4 }}>{medals[i]}</div>
                      <Avatar name={p.name} size={32} isMe={p.id === me.id} />
                      <div style={{ marginTop: 6, fontSize: 12, fontWeight: 600, color: p.id === me.id ? '#A89DF0' : '#fff' }}>{p.name.split(' ')[0]}</div>
            ...
            {sortedPlayers.map((p, i) => (
              <div key={p.id} style={{ background: p.id === me.id ? 'rgba(124,110,232,0.08)' : '#18182A', border: `0.5px solid ${p.id === me.id ? 'rgba(124,110,232,0.25)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 12, padding: '11px 1rem', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ width: 26, textAlign: 'center', fontSize: i < 3 ? 16 : 12, color: 'rgba(255,255,255,0.3)', fontWeight: 600 }}>
                  {i < 3 ? ['🥇','🥈','🥉'][i] : `#${i+1}`}
                </div>
                <Avatar name={p.name} size={34} isMe={p.id === me.id} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 600, color: p.id === me.id ? '#A89DF0' : '#fff', display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 1 }}>{p.dept} · {Object.keys(p.predictions || {}).length} picks</div>
                </div>
                <div style={{ fontSize: 18, fontWeight: 700, color: '#A89DF0', minWidth: 40, textAlign: 'right' }}>{p.points}</div>
              </div>
            ))}
            <div style={{ textAlign: 'center', fontSize: 12, color: 'rgba(255,255,255,0.2)', marginTop: 12 }}>10 pts per correct prediction</div>
          </div>
        )}
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
              {['matches', 'results', 'players'].map(t => (
                <button key={t} onClick={() => setAdminTab(t)} style={{ padding: '5px 14px', borderRadius: 999, border: '0.5px solid rgba(255,255,255,0.15)', background: adminTab === t ? '#7C6EE8' : 'transparent', color: adminTab === t ? '#fff' : 'rgba(255,255,255,0.45)', fontSize: 13, cursor: 'pointer', textTransform: 'capitalize' }}>{t}</button>
              ))}
            </div>

            {adminTab === 'matches' && (
              <div>
                <div style={{ background: '#18182A', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: '1.25rem', marginBottom: 16 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'rgba(255,255,255,0.5)', marginBottom: 14 }}>Add new match</div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 10 }}>
                    {['p1','p2'].map((s, i) => (
                      <div key={s} style={{ background: 'rgba(255,255,255,0.03)', borderRadius: 10, padding: '10px 12px' }}>
                        <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.6px' }}>Player {i+1}</div>
                ...
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', marginBottom: 10 }}>All matches ({matches.length})</div>
                {matches.map(m => (
                  <div key={m.id} style={{ background: '#18182A', border: '0.5px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '10px 1rem', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 600 }}>{m.p1.name} vs {m.p2.name}</div>
                      <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 2 }}>{m.game} · {m.type} · {m.round} · {m.date}</div>
                    </div>
                    <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 999, background: m.status === 'completed' ? 'rgba(29,158,117,0.12)' : 'rgba(255,255,255,0.06)', color: m.status === 'completed' ? '#1D9E75' : 'rgba(255,255,255,0.4)' }}>{m.status}</span>
                    <button onClick={() => deleteMatch(m.id)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.25)', cursor: 'pointer', fontSize: 18, padding: 4 }}>✕</button>
                  </div>
                ))}
              </div>
            )}
            ...
            {adminTab === 'results' && (
              <div>
                <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.35)', marginBottom: 14 }}>Select the winner — points update instantly for all predictors.</p>
                {upcoming.map(m => (
                  <div key={m.id} style={{ background: '#18182A', border: '0.5px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: '1rem 1.25rem', marginBottom: 12 }}>
                    <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                      <Badge label={m.game} style={GAME_COLORS[m.game]} />
                      <Badge label={m.type} style={TYPE_COLORS[m.type]} />
                      <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', marginLeft: 'auto' }}>{m.date} {m.time}</span>
                    </div>
            ...
            {adminTab === 'players' && (
              <div>
                <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', marginBottom: 14 }}>{sortedPlayers.length} players registered</div>
                {sortedPlayers.map((p, i) => (
                  <div key={p.id} style={{ background: '#18182A', border: '0.5px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '10px 1rem', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
                    <Avatar name={p.name} size={32} />
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 600 }}>{p.name}</div>
                      <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>{p.dept} · {Object.keys(p.predictions||{}).length} predictions</div>
                    </div>
      ...
      {/* ── Bottom nav ── */}
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#18182A', borderTop: '0.5px solid rgba(255,255,255,0.07)', display: 'flex', justifyContent: 'space-around', padding: '8px 0 14px' }}>
        {NAV.map(n => (
...
function SectionTitle({ children }) {
  return <div style={{ fontSize: 14, fontWeight: 600, color: 'rgba(255,255,255,0.5)', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '0.6px' }}>{children}</div>;
}

function EmptyState({ msg, sub }) {
  return (
    <div style={{ textAlign: 'center', padding: '3rem 1rem' }}>
      <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.3)', marginBottom: 4 }}>{msg}</div>
...
function LoadingScreen() {
  return (
    <div style={{ minHeight: '100vh', background: '#0F0F1A', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.3)', fontSize: 14 }}>
...
function AdminInput({ label, value, onChange, type = 'text', style: s }) {
  return (
    <div style={s}>
      <label style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{label}</label>
      <input type={type} value={value} onChange={e => onChange(e.target.value)} style={{ width: '100%', padding: '7px 10px', borderRadius: 8, border: '0.5px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: '#fff', fontSize: 13 }} />
    </div>
  );
}
...
function AdminSelect({ label, value, onChange, opts, wide, style: s }) {
  return (
    <div style={{ gridColumn: wide ? 'span 2' : undefined, ...s }}>
      <label style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', display: 'block', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{label}</label>
      <select value={value} onChange={e => onChange(e.target.value)} style={{ width: '100%', padding: '7px 10px', borderRadius: 8, border: '0.5px solid rgba(255,255,255,0.1)', background: '#18182A', color: '#fff', fontSize: 13 }}>
...
function MatchCard({ match: m, picked, onPick, locked }) {
  return (
    <div style={{ background: '#18182A', border: '0.5px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '1rem 1.25rem', marginBottom: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
        <Badge label={m.game} style={GAME_COLORS[m.game]} />
        <Badge label={m.type} style={TYPE_COLORS[m.type]} />
        <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)', marginLeft: 'auto' }}>{m.round}</span>
        <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.25)' }}>· {m.time}</span>
      </div>
        ...
        {['p1','p2'].map((side, i) => {
          ...
          return (
            <div key={side} onClick={() => !locked && onPick(m.id, side)}
              style={{ padding: '10px 10px', borderRadius: 10, border: `1.5px solid ${sel ? '#7C6EE8' : 'rgba(255,255,255,0.06)'}`, background: sel ? 'rgba(124,110,232,0.12)' : 'rgba(255,255,255,0.025)', cursor: locked ? 'default' : 'pointer', transition: 'all 0.15s', textAlign: i===1 ? 'right' : 'left' }}>
              <div style={{ display: 'flex', flexDirection: i===1?'row-reverse':'row', alignItems: 'center', gap: 8 }}>
                ...
                <div style={{ textAlign: i===1?'right':'left', minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: sel ? '#A89DF0' : '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{player.name}</div>
                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 1 }}>{player.dept}</div>
                </div>
              </div>
            </div>
          );
        })}
        <div style={{ textAlign: 'center', fontSize: 11, color: 'rgba(255,255,255,0.2)', fontWeight: 600 }}>VS</div>
      </div>
      <div style={{ fontSize: 12, textAlign: 'center', color: picked ? '#A89DF0' : 'rgba(255,255,255,0.2)' }}>
